...
gcloud scheduler jobs create http grace-irvine-ics-updater \
    ...
    --http-method=POST \
    --headers="X-Auth-Token=REDACTED_SECRET" \
...
```bash
# 测试部署的服务
python test_api_endpoints.py https://YOUR-SERVICE-URL REDACTED_SECRET
